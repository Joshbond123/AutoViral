import { supabase } from './supabase';
  ...
  export async function fetchHistory(userId: string): Promise<Post[]> {
    if (!supabase) return [];
    // Select only known columns. Do NOT sort by created_at — it may not exist.
    const { data, error } = await supabase
      .from('posts')
  ...
  export async function fetchSchedules(userId: string): Promise<Schedule[]> {
    ...
    if (error) {
      console.warn('fetchSchedules error:', error.message);
      return [];
    }
    return (data ?? []) as Schedule[];
  }
  ...
  /**
   * Count ALL schedules for the user (pending, running, success, failed).
   * This shows the total number of automation schedules configured —
   * not just the ones currently waiting to run.
   */
  export async function fetchScheduledCount(userId: string): Promise<number> {
    ...
    const { count, error } = await supabase
      ...
      .eq('user_id', userId);
    if (error) {
      console.warn('fetchScheduledCount error:', error.message);
      return 0;
    }
    return count ?? 0;
  }
