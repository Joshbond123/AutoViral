import { supabase } from './supabase';
    ...
    /**
     * Server-side logout:
     * 1. Calls the tiktok-logout edge function which revokes the TikTok token
     *    and deletes all user data from Supabase.
     * 2. Best-effort — never throws, so the client can always clear local state.
     */
    export async function logoutTikTok(userId: string): Promise<void> {
      ...
      try {
        ...
        // Non-fatal — client will still clear local state
      }
    }
