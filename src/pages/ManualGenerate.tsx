...
import {
  ...
  BellOff, BellRing, ShieldCheck, Edit2, CheckCircle2, Share2,
} from 'lucide-react';
import {
...
function VideoCard({ post, onDelete }: { post: Post; onDelete: (id: string) => void }) {
  ...
  const [publishedFb, setPublishedFb] = useState(false);
  ...
  const handlePublishToFacebook = async () => {
    if (publishingFb || publishedFb || !post.video_url) return;
    const userId = localStorage.getItem('tiktok_user_id') || '';
    if (!userId) { setFbError('Not logged in'); return; }
    setPublishingFb(true);
    setFbError(null);
    try {
      const result = await publishPostToFacebook(userId, {
        id: post.id,
      ...
      if (result.ok) {
        setPublishedFb(true);
        setTimeout(() => setPublishedFb(false), 5000);
      } else {
        setFbError(result.error?.slice(0, 80) ?? 'No Facebook Page configured — add one in Settings');
        setTimeout(() => setFbError(null), 5000);
      }
    } catch (e: any) {
      setFbError(e.message?.slice(0, 80) ?? 'Publish failed');
      setTimeout(() => setFbError(null), 4000);
    } finally {
          ...
          <div className="flex items-center gap-1.5 pt-1 mt-auto flex-wrap">
            ...
            {post.video_url && isReady && (
              <button
                ...
                disabled={publishingFb || publishedFb}
                title={fbError ?? (publishedFb ? 'Published to Facebook' : 'Publish to Facebook Page')}
                className={`flex items-center gap-1.5 px-3 py-2 rounded-xl transition-all text-xs font-medium disabled:opacity-60 ${
                  publishedFb
                    ...
                    : 'bg-white/5 hover:bg-blue-600/10 text-white/40 hover:text-blue-400'
                }`}
              >
                {publishingFb
                ...
                <span className="hidden sm:inline">
                  {publishingFb ? 'Publishing…' : publishedFb ? 'Published!' : 'Publish to Facebook'}
                </span>
              </button>
            )}
            <button
